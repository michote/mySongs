/**
	CombinedSlider is a variation on an _onyx.RangeSlider_ which also allows a simple slider mode
  (1) A range slider control that combines a horizontal slider with two
	control knobs. The user may drag the knobs to select a desired range of
	values.
  (2) A simple slider where the user may drag the knob to select the desired value. 
  The first knob of the range slider is set invisible and rangeStart to rangeMin.
  
  Set isSimple true to use as a simple (one knob) slider.

		{kind: "CombinedSlider", rangeMin: 100, rangeMax: 500,
...
enyo.kind({
	...
	published: {
		...
		*/
		...
		endValue: 0,
    // Use as simple slider  - start == end
    isSimple: false,
	},
	//* @public
	events: {
    ...
    if (this.isSimple) {
      this.$.startKnob.applyStyle("visibility", "hidden");
      this.rangeStart = this.rangeMin;;
    } else {
      this.$.startKnob.applyStyle("visibility", "visible");
    }
	...
	drag: function(inSender, inEvent) {
		if (this.dragging) {
			...
			var _val, val, p;
      if ((inSender.name === "startKnob") && (knobPos >= 0) && (!this.isSimple)) {
